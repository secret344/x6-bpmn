...
interface FieldConfig {
  key: string; label: string; type: 'text' | 'textarea' | 'boolean' | 'select'
...
const bpmnFields = computed<FieldConfig[]>(() => {
  ...
  if (cat === 'userTask') {
    fields.push(
      ...
      { key: 'formKey', label: '表单Key', type: 'text' },
      { key: 'dueDate', label: '到期日', type: 'text' },
  ...
  if (cat === 'serviceTask' || cat === 'businessRuleTask' || cat === 'sendTask' || cat === 'receiveTask') {
    fields.push(
      { key: 'implementationType', label: '实现类型', type: 'select', options: [
        { value: 'class', label: 'Java 类' }, { value: 'expression', label: '表达式' },
  ...
  if (cat === 'scriptTask') {
    fields.push(
      { key: 'scriptFormat', label: '脚本格式', type: 'select', options: [
        { value: 'groovy', label: 'Groovy' }, { value: 'javascript', label: 'JavaScript' },
      ]},
  ...
  if (cat === 'callActivity') {
    fields.push({ key: 'calledElement', label: '被调流程', type: 'text' }, { key: 'isAsync', label: '异步', type: 'boolean' })
  }
  if (cat === 'subProcess') {
    fields.push({ key: 'isAsync', label: '异步', type: 'boolean' }, { key: 'triggeredByEvent', label: '事件触发', type: 'boolean' })
  }
  if (cat === 'gateway') {
    fields.push({ key: 'defaultFlow', label: '默认流', type: 'text' }, { key: 'activationCondition', label: '激活条件', type: 'text' })
  }
  if (cat === 'timerEvent') {
    fields.push(
      { key: 'timerType', label: '定时类型', type: 'select', options: [
        { value: 'timeDuration', label: '持续时间' }, { value: 'timeDate', label: '固定时间' }, { value: 'timeCycle', label: '循环' },
      ]},
      { key: 'timerValue', label: '定时值', type: 'text', placeholder: 'PT5M' },
    )
  }
  if (cat === 'messageEvent') { fields.push({ key: 'messageRef', label: '消息引用', type: 'text' }, { key: 'messageName', label: '消息名称', type: 'text' }) }
  if (cat === 'signalEvent') { fields.push({ key: 'signalRef', label: '信号引用', type: 'text' }, { key: 'signalName', label: '信号名称', type: 'text' }) }
  if (cat === 'errorEvent') { fields.push({ key: 'errorRef', label: '错误引用', type: 'text' }, { key: 'errorCode', label: '错误代码', type: 'text' }) }
  if (cat === 'escalationEvent') { fields.push({ key: 'escalationRef', label: '升级引用', type: 'text' }, { key: 'escalationCode', label: '升级代码', type: 'text' }) }
  if (cat === 'conditionalEvent') { fields.push({ key: 'conditionExpression', label: '条件', type: 'text' }) }
  if (cat === 'linkEvent') { fields.push({ key: 'linkName', label: '链接名', type: 'text' }) }
  if (cat === 'compensationEvent') { fields.push({ key: 'activityRef', label: '活动引用', type: 'text' }) }
  if (cat === 'dataObject') { fields.push({ key: 'isCollection', label: '集合', type: 'boolean' }) }
  if (cat === 'pool') { fields.push({ key: 'processRef', label: '流程引用', type: 'text' }) }
  if (cat === 'textAnnotation') { fields.push({ key: 'annotationText', label: '注释', type: 'textarea' }) }
  if (cat === 'group') { fields.push({ key: 'categoryValueRef', label: '分类值', type: 'text' }) }
  if (selectedCell.value?.shape.includes('boundary')) { fields.push({ key: 'cancelActivity', label: '中断', type: 'boolean' }) }
  return fields
})
...
watch(() => props.graph, (g) => {
  ...
  const onBlank = () => { selectedCell.value = null; cellPos.value = null; cellSize.value = null; isEdge.value = false; labelValue.value = ''; customAttrs.splice(0, customAttrs.length) }
